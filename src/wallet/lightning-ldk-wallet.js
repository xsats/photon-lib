...
import { BitcoinUnit, Chain } from '../model/bitcoinUnits';
...
const RnLdk = require('rn-ldk/src/index');
...
export class LightningLdkWallet extends LightningCustodianWallet {
  ...
  _listPayments = [];
  _listInvoices = [];
  _nodeConnectionDetailsCache = {}; // pubkey -> {pubkey, host, port, ts}
  _refundAddressScriptHex = '';
  _lastTimeBlockchainCheckedTs = 0;
  _unwrapFirstExternalAddressFromMnemonicsCache = '';
  _predefinedNodes = {
  ...
  static pubkeyToAlias(pubkeyHex) {
  ...
  async openChannel(pubkeyHex, host, amountSats, staticChannel) {
  ...
  async init() {
    ...
    try {
      ...
      console.warn('LDK init error: ' + error.message);
    }
  }
   ...
   *
   * @return {Promise<{confirmedBalance}>}
   */
  async walletBalance() {
    let confirmedSat = 0;
   ...
   *
   * @param func {function} Async function to execute
   * @static
   */
  _execInBackground(func) {
    ...
    (async () => {
      try {
        ...
        console.warn('_execInBackground error:' + error.message);
      }
    })();
  }
}
