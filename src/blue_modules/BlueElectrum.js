...
import { LegacyWallet, SegwitBech32Wallet, SegwitP2SHWallet, TaprootWallet } from '../wallet';
...
export const multiGetTransactionByTxid = async function (txids, batchsize, verbose = true) {
 ...
 * @returns {number}
 */
 export const calcEstimateFeeFromFeeHistorgam = function (numberOfBlocks, feeHistorgram) {
...
export const estimateFees = async function () {
  ...
  const fast = calcEstimateFeeFromFeeHistorgam(1, histogram);
...
export const testConnection = async function (host, tcpPort, sslPort) {
  ...
  try {
    const rez = await Promise.race([
      new Promise(resolve => {
        timeoutId = setTimeout(() => resolve('timeout'), // host.endsWith('.onion') && !isTorDisabled ? 21000 :
         5000);
      }),
      client.connect(),
    ]);
